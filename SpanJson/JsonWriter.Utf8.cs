...
namespace SpanJson
{
    public ref partial struct JsonWriter<TSymbol> where TSymbol : struct
    {
        ...
        public void WriteUtf8Single(float value)
        {
            ...
            foreach(ref readonly var c in span[..written])
            {
                _bytes[pos++] = (byte)c;
            }
        }
        ...
        public void WriteUtf8Decimal(decimal value)
        {
            ...
            span[..bytesWritten].CopyTo(_bytes[pos..]);
            pos += bytesWritten;
        }
        ...
        public void WriteUtf8DateTime(DateTime value)
        {
            ...
            WriteUtf8DoubleQuote();
            DateTimeFormatter.TryFormat(value, _bytes[pos..], out var bytesWritten);
            pos += bytesWritten;
            WriteUtf8DoubleQuote();
        }
        ...
        public void WriteUtf8TimeSpan(TimeSpan value)
        {
            ...
            WriteUtf8DoubleQuote();
            Utf8Formatter.TryFormat(value, _bytes[pos..], out var bytesWritten);
            pos += bytesWritten;
            WriteUtf8DoubleQuote();
        }

        public void WriteUtf8DateOnly(DateOnly value)
        {
            ref var pos = ref _pos;
            const int tsSize = JsonSharedConstant.MaxDateOnlyLength;
            if (pos > _bytes.Length - tsSize)
            {
                Grow(tsSize);
            }
        ...
        public void WriteUtf8TimeOnly(TimeOnly value)
        {
            ref var pos = ref _pos;
            const int tsSize = JsonSharedConstant.MaxTimeOnlyLength;
            if (pos > _bytes.Length - tsSize)
            {
                Grow(tsSize);
            }
        ...
        public void WriteUtf8String(in ReadOnlySpan<char> value)
        {
            ...
            while (index < value.Length)
            {
                ...
                if (c < 0x20 || c == JsonUtf8Constant.DoubleQuote || c == JsonUtf8Constant.Solidus || c == JsonUtf8Constant.ReverseSolidus)
                {
                    ...
                    WriteUtf8ReadonlySpan(value[from..index], 0);
                    if (pos > _bytes.Length - 7) // The EscapedUtf8Char takes at most 6 bytes + 1 for double quote
                    {
                        Grow(7);
                    }
            ...
            {
                WriteUtf8ReadonlySpan(value[from..], 1); // make sure we have place for doublequote
            }
        ...
        public void WriteUtf8Version(Version value)
        {
            ...
            pos += Encoding.UTF8.GetBytes(tempSpan[..written], _bytes[pos..]);
            WriteUtf8DoubleQuote();
        }
        ...
        public void WriteUtf8Base64EncodedArray(in ReadOnlySpan<byte> value)
        {
            ...
            WriteUtf8DoubleQuote();
            if (Base64.EncodeToUtf8(value, _bytes[pos..], out _, out var written) != OperationStatus.Done)
            {
